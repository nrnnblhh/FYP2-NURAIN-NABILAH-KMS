...
if (isset($_GET['token'])) {
    ...
    $stmt = $conn->prepare("SELECT * FROM teacher WHERE resetToken = ? AND tokenExpiry > NOW()");
    $stmt->bind_param('s', $token);
    $stmt->execute();
    $result = $stmt->get_result();

    if ($result->num_rows > 0) {
        // Token is valid; get the teacher record
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            if (empty($newPassword) || empty($confirmPassword)) {
                ...
                $stmt = $conn->prepare("UPDATE teacher SET tPass = ?, resetToken = NULL, tokenExpiry = NULL WHERE resetToken = ?");
                $stmt->bind_param('ss', $hashedPassword, $token);

                if ($stmt->execute()) {
                    $success = "Your password has been reset successfully. You can now <a href='teacherlogin.php'>log in</a>.";
